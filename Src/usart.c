...
uint8_t tcp_server_ip[] = "21.tcp.cpolar.top";
uint16_t tcp_server_port = 11919;
uint8_t tcp_client_socket = 0;

uint8_t REMOTE_MAC[64] = "0xBA03454151C1";
uint8_t gIMEI[16];		//设备唯一标识
...
//	HAL_UART_Receive_IT(&hlpuart1, (uint8_t *)lp1_aRxBuffer, RXBUFFERSIZE);
...
//	HAL_Delay(1500);
	...
	//查询通信模块IMEI
	print_u1("AT+CGSN=1\r\n");
	uint8_t debug_msg[70] = {0};
	uint8_t imei_buffer[64] = {0};
	
	//判断是否触发了ORE错误
	if(__HAL_UART_GET_FLAG(&huart1, UART_CLEAR_OREF) != RESET)
	{
			//取出串口缓存中多余的字节数
	...
	HAL_StatusTypeDef status = HAL_UART_Receive(&huart1,imei_buffer,64, DELAY_TIME);
	__HAL_UART_FLUSH_DRREGISTER(&huart1);	
	parseImei(imei_buffer);
	//调试打印
	sprintf((char*)debug_msg,"IMEI req res:%s\r\nstatus:%x\r\nIMEI:%s\r\n",imei_buffer,status,gIMEI);
	print_u1(debug_msg);
}

void parseImei(uint8_t *msg_buffer){
		const char* targetstr = "CGSN:";
		const char* pidx0 = strstr((char *)msg_buffer, targetstr);
		if(pidx0 == NULL)
			return;
		strncpy((char*)gIMEI, pidx0+strlen(targetstr)+1, 15);
}
